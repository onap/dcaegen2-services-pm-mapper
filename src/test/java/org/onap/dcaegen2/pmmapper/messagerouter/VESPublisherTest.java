...
public class VESPublisherTest {
    ...
    public void publish_success() throws Exception {
        sut.publish(ves);
        verify(sender, times(1)).send("POST",topicURL, ves);
    }
    ...
    @Test
    public void publish_fail() throws Exception {
        when(sender.send(Mockito.anyString(),Mockito.anyString(),Mockito.anyString())).thenThrow(Exception.class);
        assertThrows(MRPublisherException.class, () -> sut.publish(ves));
    }
