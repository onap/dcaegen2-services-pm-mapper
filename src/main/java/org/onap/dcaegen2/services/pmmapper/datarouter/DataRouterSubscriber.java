...
public class DataRouterSubscriber implements HttpHandler {
    ...
    public DataRouterSubscriber(EventReceiver eventReceiver) {
        ...
        this.metadataBuilder = new GsonBuilder().registerTypeAdapter(EventMetadata.class, new RequiredFieldDeserializer<EventMetadata>()).create();
    }
    ...
    private void subscribe(int attempts, int timeout, BusControllerConfig config) throws TooManyTriesException, InterruptedException {
        ...
        String subMessage = "";
        try {
            HttpURLConnection connection = (HttpURLConnection) config.getDataRouterSubscribeEndpoint().openConnection();
            ...
            try (OutputStream bodyStream = connection.getOutputStream();
                ...
                bodyWriter.write(subscriberObj.toString());
            }
    ...
    public void handleRequest(HttpServerExchange httpServerExchange) {
        if (limited) {
            httpServerExchange.setStatusCode(StatusCodes.SERVICE_UNAVAILABLE).getResponseSender().send(StatusCodes.SERVICE_UNAVAILABLE_STRING);
        } else {
            try {
                EventMetadata metadata = metadataBuilder.fromJson(httpServerExchange.getRequestHeaders().get(METADATA_HEADER).get(0), EventMetadata.class);
                httpServerExchange.getRequestReceiver().receiveFullString((callbackExchange, body) -> {
                ...
                httpServerExchange.setStatusCode(StatusCodes.BAD_REQUEST).getResponseSender().send(NO_METADATA_MESSAGE);
            } catch (JsonParseException exception) {
                log.info("Bad Request: Failure to parse metadata", exception);
                httpServerExchange.setStatusCode(StatusCodes.BAD_REQUEST).getResponseSender().send(BAD_METADATA_MESSAGE);
            }
        }
    }
}
