...
public class App {
    ...
    private boolean isCached(Event event) {
        boolean isPresent = eventsCache.contains(event);
        if(isPresent) {
            logger.unwrap().info("Skipping. This event is already waiting in cache to be processed: ", event.getBody());
        }
        return isPresent;
    }
