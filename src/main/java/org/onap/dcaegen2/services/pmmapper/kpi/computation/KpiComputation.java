...
public class KpiComputation {
    ...
    public static List<String> checkAndDoComputation(String ves, MapperConfig config) {
        ...
        String eventName = Optional.of(pmEvent)
                    .map(PerformanceEvent::getCommonEventHeader)
        ...
        kpis.forEach(k -> {
            ...
            switch (operation) {
                case SUM:
                    result = kpiOperands.stream().map(i -> i).reduce(BigDecimal.ZERO, BigDecimal::add);
                    ...
                    measTypesList.add(new StringBuilder().append(k.getMeasType()).append(keyOperands).toString());
                    ...
                    PerformanceEvent kpiPMEvent = new PerformanceEvent();
