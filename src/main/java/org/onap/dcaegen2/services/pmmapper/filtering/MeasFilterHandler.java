...
public class MeasFilterHandler {
    ...
    private List<Pattern> getPatternsFromFilters(Optional<Filter> filter) {
        return filter.get().getMeasTypes().stream().map(measType -> Pattern.compile("^"+measType+"$")).collect(Collectors.toList());
    }
